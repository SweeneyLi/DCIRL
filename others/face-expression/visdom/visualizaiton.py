...
class Visualization(object):
  def __init__(self, opt):
    self.viz = Visdom(port=5559, server='http://localhost', base_url='/', username='',password='', use_incoming_socket=True, env=opt.visual_env)
    ...
    self.train_loss_win = self.viz.line(
            ...
            opts={'title':'train curve',
                  'xlabel':'iteration',
                  'ylabel':'loss',
                  'legend':['loss', 'cls_loss', 'triplet_loss', 'accuracy'],
                  'width':1024,
                  'height':256,
                  'showlegend':True})
    self.downsample = nn.Upsample(scale_factor=1./self.scale, mode='bilinear', align_corners=True)

  def run(self, samples, loss, cls_loss, triplet_loss, accuracy, predicted, labels, cnt):
    loss = [np.array(loss)]
    self.viz.line(X=np.column_stack(([cnt], [cnt], [cnt], [cnt])),
                  Y=np.column_stack((loss, cls_loss, triplet_loss, accuracy)),
                  win=self.train_loss_win,
                  update='append')
    post_samples = self.post_processing(samples, predicted, labels)
    self.viz.images(post_samples.numpy(),
                    nrow=self.nrow,
                    win=self.sample_win,
                    opts={'title':'Samples Domain'})
  def post_processing(self, image, predicted, labels):
